...
export default function Ranks() {
    ...
    const dedinList = ['DeDIN Alliance-Litentry', 'DeDIN Alliance-Mantle', 'DeDIN Alliance-Hippocrat']
    ...
    const dedinSelect = (id) => {
        for(let i=0; i < dedinList.length; i++) {
            document.getElementById(`dedin_${i}`).style.color = "white"
        }
        document.getElementById(`dedin_${id}`).style.color = "#05F292"
    }
            ...
            <div className="px-6 md:px-12 lg:px-20 relative">
                ...
                <div className="bg-[#05F29205] border-x-4 border-solid border-[#05F292] pl-0 rounded-2xl my-12 relative flex gap-6 md:flex-row z-10">
                    <div ref={ side } className="hidden lg:flex flex-col gap-0 bg-[#024025] rounded-2xl p-4">
                        ...
                        { dedinList.map((dedin, i) => {
                            return (
                                <p onClick={ () => dedinSelect(i) } key={i} id={`dedin_${i}`} className="dedin_iter w-full p-4 border-b border-dashed border-xspace-border">{ dedin }</p>
                            )
                        })}
                    </div>
                    ...
                    <div ref={ main } className="lg:w-4/5 p-4">
                        ...
                        <section className="flex justify-between items-center gap-4">
                            <div className="">
                                <h2 className="text-[#05F292]">DeDIN Alliance-Litentry</h2>
                                <p className="">Sep 25- Oct 10</p>
                            </div>
